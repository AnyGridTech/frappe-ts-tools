...
/**
 * Bundle all public TypeScript files into a single JavaScript file
 * @param {Object} publicFolder - Public folder configuration
 * @param {boolean} watch - Enable watch mode
 * @param {Object} options - Build options
 */
function bundlePublicTS(publicFolder, watch = false, options = {}) {
  const {
    external = ['frappe', 'jquery'],
    format = 'iife',
    target = 'es2020',
  ...
  // Create a temporary entry file that imports all .ts files
  const tempEntry = path.join(publicFolder.tsPath, '__bundle_entry__.ts');
  const imports = publicFolder.tsFiles
    .map(f => `import "./${f.replace('.ts', '')}";`)
    .join('\n');
  
  fs.writeFileSync(tempEntry, imports);
  
  const outputFile = path.join(publicFolder.jsPath, `${bundleName}.bundle.js`);
  const externalArgs = external.map(e => `--external:${e}`).join(' ');
  ...
  const cmd = [
    ...
    path.relative(process.cwd(), tempEntry),
    '--bundle',
    `--outfile=${path.relative(process.cwd(), outputFile)}`,
    `--format=${format}`,
    `--target=${target}`,
    externalArgs,
    `--banner:js="${banner}"`,
  ...
  console.log(`Bundling ${publicFolder.tsFiles.length} public TypeScript file(s) into ${bundleName}.bundle.js...`);
...
/**
 * Clean up temporary bundle entry files from public folders
 * @param {Array} publicFolders - Array of public folder configurations
 */
function cleanupPublicBundles(publicFolders) {
  console.log('Cleaning up public bundles...');
  publicFolders.forEach(folder => {
    const tempEntry = path.join(folder.tsPath, '__bundle_entry__.ts');
    if (fs.existsSync(tempEntry)) {
      fs.unlinkSync(tempEntry);
    }
  });
